...
this.walletAlltranstactions()
  
  }


  walletAlltranstactions(){
    this.user_id = localStorage.getItem('VendorId');
         let param = { 
           user_id:2   
         }
         this.http
           .post("http://139.59.21.147:8080/api/wallets/wallet_by_user", param)
           .subscribe((response: any) => {
             ...
             if (response.statuscode == '404') {
              ...
              //  this.JsonData = response.data[0].balance;
               console.log("datata",response.data[0].balance)
               // this.disabled=false;
  ...
  onFilterBy(deviceValue:any) {
    
    alert()
    this.user_id = localStorage.getItem('VendorId');
    let param = { 
      filter_type:deviceValue.target.value,
      user_id:1
      // user_id:this.user_id
      
    } 
    console.log("filter_type",deviceValue.value)   
    this.http
      .post("http://139.59.21.147:8080/api/wallets/wallet_filter", param)
      .subscribe((response: any) => {
        if (response.statuscode == '404') {
          alert(response.message);
        }
          ...
          // alert(response.message);
          this.JsonData = response.data;  
          console.log("length00",this.JsonData)
        }
      });
